...
\begin{definition}[Syntax of $\mathtt{(un)SecureND}$]\label{def:syntax} %The syntax  is defined by the following alphabet:
	...
	\mathcal{C}:= \{C^{S_1}_{\overrightarrow{n}}\leq \dots \leq C^{S_{n}}_{\overrightarrow{n}}\}\\  %\sim\in \{<,\leq, =, \geq, >\}
...
An order between messages $\phi^{v_{j}}_{i,k}<\phi^{v_{j}}_{i,l}$ holds if $C_{k}\leq C_{l} \in S_{i}$ for a vehicle $v_{j}$.
%
...
Any message $\phi^{i,k}$ valid for a channel $v_{i},v_{j}$ and obtained by an occurrence $c$ of the $Cut$ rule can be validated without $c$ using only trust.	
\end{theorem}
...
In this section we present the algorithm and example derivations for handshaking and opportunistic message forwarding protocols. The algorithm consists of two parts: it first implements the recipient selection of a communication by reputation and then message forwarding if consistency is guaranteed by trust. The full protocol with handshaking and opportunistic forwarding is formulated in Figure \ref{fig:routine1}. Here we use protocol operations named after the relevant $\mathtt{SecureND}$ rules, vehicles, services and characteristics variables.
