...
% FR minor point: changed index for agent i to differentiate it from service i
\begin{definition}[Formulae]
A formula $\Gamma^{v}_{l} \vdash_{\mathtt{s}} \phi^{v_{j}}_{i,k}$ states that a message $\phi$ about service $i$ and characteristic $k$ signed from agent $v_{j}$  is validly accessed at step $\mathtt{s}\geq 0$ under the profile of agent $v_{l}$.
\end{definition}
